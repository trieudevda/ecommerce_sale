...
import {getListProductColumns, ProductDataType} from "@/components/templates/admin/product/listColumn";
...
const COLUMN_OPTIONS = [
    ...
    { label: 'Mô tả ngắn', value: 'short_description' },
...
const App: React.FC = () => {
    ...
    // State quản lý Cài đặt hiển thị cột
    const [isSettingsOpen, setIsSettingsOpen] = useState(false);
    const [visibleColumns, setVisibleColumns] = useState<string[]>(
        COLUMN_OPTIONS.map(col => col.value)
    );

    const onChange: TableProps<ProductDataType>['onChange'] = (pagination, filters, sorter, extra) => {
    ...
    const handleEdit = (slug: string) => router.push(ADMIN_PATHS.PRODUCT.EDIT(slug));
    ...
    const handleDelete = async (slug: string) => {
        ...
        if (data?.statusCode && data.statusCode >= 400) {
            ...
            messageApi.success({ title: tMess('Title.info'), description: tMess('Description.data_updated_successfully') });
            setProductList((prevData) => prevData.filter(p => p.slug !== slug));
        }
    };

    const handleCreate = () => router.push(ADMIN_PATHS.PRODUCT.CREATE());
    ...
    const filteredProducts = useMemo(() => {
        ...
        return productList.filter(p =>
            ...
            (p.short_description && p.short_description.toLowerCase().includes(searchText.toLowerCase()))
        );
        ...
        <div className="flex flex-col gap-4 p-1 w-56">
            <div>
                ...
                <Checkbox.Group className="flex flex-col gap-2.5">
                    <Checkbox value="active" className="text-slate-600 hover:text-slate-900 transition-colors">Đang kinh doanh</Checkbox>
                    <Checkbox value="inactive" className="text-slate-600 hover:text-slate-900 transition-colors">Tạm ẩn</Checkbox>
                    <Checkbox value="out_of_stock" className="text-slate-600 hover:text-slate-900 transition-colors">Hết hàng</Checkbox>
                </Checkbox.Group>
            </div>
            <div className="border-t border-slate-100 pt-3 flex justify-end gap-2">
                <Button size="small" type="text" className="text-slate-500 hover:bg-slate-100">Xóa</Button>
                <Button size="small" type="primary" className="bg-blue-600 shadow-md shadow-blue-500/20">Áp dụng</Button>
            </div>
        </div>
    );
                    ...
                    <div className="flex gap-4 items-center">
                        ...
                        <div>
                            <Typography.Title level={2} className="!mb-0 !text-xl !font-extrabold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-slate-900 to-slate-600">
                                {t('title') || 'Danh sách sản phẩm'}
                            </Typography.Title>
                            <p className="text-slate-500 text-sm font-medium mt-1">
                                Quản lý kho hàng, theo dõi trạng thái và cập nhật thông tin sản phẩm.
                            </p>
                        </div>
                    </div>
                ...
                >
                    <div className="bg-slate-50 px-6 py-5 border-b border-slate-100">
                        ...
                        <p className="text-sm text-slate-500 mt-2 ml-11">
                            {tModal('enabledisable_data_fields_to_optimize_warehouse_display_space')}
                        </p>
                    </div>
